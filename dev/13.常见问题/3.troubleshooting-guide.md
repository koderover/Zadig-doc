...
## Helm 类型项目工作流部署失败，如何解决
当工作流部署 Helm 服务遇到`cannot re-use a name that is still in use`或者`another operation (install/upgrade/rollback) is in progress`报错时，Helm release 可能因为一些异常情形处于不正确的状态。可以按照以下情况解决：
* 如果 release 存在状态为 deployed 的版本，手动将 release rollback 到最近的 deployed 版本。
* 如果 release 当前状态处于 superseded 状态，并且 release 不存在状态为 deployed 的版本，手动执行 helm upgrade 命令升级到新版本。
